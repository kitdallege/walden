...
void configurator_zero(Configurator *self)
{
	...
	memset(self->conf->web_root, 0, CONF_VALUE_LEN);
...
int configurator_load_config(Configurator *self)
{
	...
	res = PQexec(conn, "select value from site_setting where name = 'renderer';");
	...
	if (!json_object_object_get_ex(obj, "web_dir", &attr)) {
		fprintf(stderr, "unable to find query_root in config json.\n");
		return 1;
	}
	strcpy(self->conf->web_root, json_object_get_string(attr));
	if (!json_object_object_get_ex(obj, "query_dir", &attr)) {
		fprintf(stderr, "unable to find web_root in config json.\n");
		return 1;
	}
	...
	fprintf(
		stderr,
		"Config {db_conn_info=%s, template_root=%s, web_root=%s, query_root=%s, site_id=%s} @ %p \n",
		self->conf->db_conn_info, self->conf->template_root,
		self->conf->web_root, self->conf->query_root,
		self->conf->site_id, (void *)self->conf
	);
	PQfinish(conn);
	PQclear(res);
	json_object_put(obj);
